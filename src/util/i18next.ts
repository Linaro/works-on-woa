...
export function getCurrentLocale(): string {
  const lang = new LanguageDetector().detect(['path', 'querystring']) || DEFAULT_LOCALE;
...
export async function initClientI18next(locale?: string): Promise<TFunction> {
  return i18n
    ...
    .init({
      lng: locale || getCurrentLocale(), // Use provided locale if available, otherwise let detector work
      fallbackLng: DEFAULT_LOCALE,
      supportedLngs: LOCALES,
      ns: ['translation'],
      defaultNS: 'translation',
